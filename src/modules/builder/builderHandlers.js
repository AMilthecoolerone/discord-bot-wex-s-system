...
export async function handleBuilderModal(interaction) {
  ...
  if (modalNumber === 1) {
    ...
    if (aiDetected) {
      ...
      if (reviewSent) {
        await interaction.reply({ 
          content: '❌ Your application has been automatically rejected due to AI usage. Any applications using AI will instantly be denied.', 
          ephemeral: true 
        });
        logger.info(`Builder application auto-rejected for AI usage: ${user.tag} (${user.id})`);
      } else {
        await interaction.reply({ 
          content: '❌ Your application was processed but there was an error sending it to the review channel. Please contact staff.', 
          ephemeral: true 
        });
      }
      return;
    ...
    if (reviewSent) {
      await interaction.reply({ 
        content: '✅ Your builder application has been submitted successfully! Staff will review it shortly.', 
        ephemeral: true 
      });
      logger.info(`Builder application completed: ${user.tag} (${user.id}) in guild ${guild.id}`);
    } else {
      await interaction.reply({ 
        content: '⚠️ Your application was saved but there was an error sending it to the review channel. Please contact staff.', 
        ephemeral: true 
      });
